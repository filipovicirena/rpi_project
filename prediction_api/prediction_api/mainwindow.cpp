    ...
    : QMainWindow(parent)
{
    ...
    blobNM = new QNetworkAccessManager(this);
    ...
    blobUploadUrl = config.blobUploadUrl;
...
void MainWindow::onPredictionResult(QNetworkReply* reply) {
    ...
    if (tag.toLower() == "intruder") {
        QString uploadUrl = uploadPhotoToBlob(photoPath);
        sendToLogicApp(tag, probability, uploadUrl);
    }
...
QString MainWindow::uploadPhotoToBlob(const QString& photoPath) {
    ...
    qDebug() << "photoPath in uploadToBlob():" << photoPath;
    qDebug() << "Extracted filename:" << filename;

    // Split the URL into base and query string
    QStringList parts = blobUploadUrl.split("?");
    if (parts.size() != 2) {
        qDebug() << "Invalid blobUploadUrl format in config.json";
        return "fail";
    }
